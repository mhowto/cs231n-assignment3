...
def rnn_step_backward(dnext_h, cache):
    ...
    dforward = (1 - next_h ** 2) * dnext_h # (N*H)

    dx = np.dot(dforward, Wx.T) # (N*D)
    dWx = np.dot(x.T, dforward) # (D*H)

    dprev_h = np.dot(dforward, Wh.T) #(N*H)
    dWh = np.dot(prev_h.T, dforward) # (H*H)

    db = np.sum(dforward, axis=0) # (H,)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ##############################################################################
    T = x.shape[1]
    inputs = np.split(x, T, axis=1)
    inputs = [np.squeeze(input) for input in inputs]
    h = []
    cache = []

    prev_h = h0
    for input in inputs:
        prev_h, c = rnn_step_forward(input, prev_h, Wx, Wh, b)
        h.append(prev_h)
        cache.append(c)
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
    ...
    dhs = np.split(dh, T, axis=1)
    dprev_h_i = 0
    for idx in reversed(range(T)):
        dh = np.squeeze(dhs[idx])
        dx_i, dprev_h_i, dWx_i, dWh_i, db_i = rnn_step_backward(dh + dprev_h_i, cache[idx])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = a, ai, af, ao, ag, i, f, o, g, next_c, next_h, x, prev_h, prev_c, Wx, Wh, b
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    a, ai, af, ao, ag, i, f, o, g, next_c, next_h, x, prev_h, prev_c, Wx, Wh, b = cache
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape
    a, ai, af, ao, ag, i, f, o, g, next_c, next_h, x, prev_h, prev_c, Wx, Wh, b = cache[0]
